...
public class TopicoController
{
    ...
    @PostMapping
    public ResponseEntity registarTopico(@RequestBody @Valid DatosRegistroTopico parametro, UriComponentsBuilder uriComponentsBuilder)
    {
    ...
    @GetMapping
    public ResponseEntity<Page<DatosListadoTopico>> listarTopicos(@PageableDefault(size = 5, sort = "fecha") Pageable paginacion)
    {
    ...
    @DeleteMapping("/{id}")
    @Transactional
    public ResponseEntity borrarTopicoDb(@PathVariable @Valid Long id, @RequestParam(name = "borrar", defaultValue = "false") @Valid boolean borrar)
    {
        if(borrar)
        {
